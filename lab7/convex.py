...
def jarvis(points, image):
    ...
    for i in range(1, len(wraping_points)):
      draw.rectangle((wraping_points[i-1][0], wraping_points[i-1][1], wraping_points[i][0], wraping_points[i][1]) , outline="green")
    ...
    draw.rectangle((wraping_points[len(wraping_points)-1][0], wraping_points[len(wraping_points)-1][1], wraping_points[0][0], wraping_points[0][1]) ,outline="green")
 
def dfs(image, inicio, visitados, output="hull.jpg"):
    pic = image.load()
    siguientes = list()
